...
Every run writes a self-contained HTML cohort report, `QC_SUMMARY/cmp_cohort_report.html`. It opens
offline and needs no internet or CDN. It shows read retention through each depletion stage per sample,
whether each sample cleared the MetaPhlAn gate, how many genera and species each profiler called, the
consensus mode, and, when they ran, the alignment-validation and decontamination summaries.

An example, generated by the integration test on **synthetic** reads (5 made-up samples: BAM, CRAM,
paired and single FASTQ, and one near-empty sample):

- [`docs/examples/cmp_cohort_report.example.html`](docs/examples/cmp_cohort_report.example.html): the
  report (download it and open it in a browser; GitHub shows HTML as source)
- [`docs/examples/cmp.qc.summary.example.tsv`](docs/examples/cmp.qc.summary.example.tsv): the QC table
  behind it, one row per sample

The same run also writes `MULTIQC/multiqc_report.html` (fastp and every FastQC pass) and Nextflow's own
`pipeline_info/report.html` and `timeline.html` (resources and timing per task).
